...
class Inventory extends AbstractSoapClient
{
    const REQUEST_INVENTORY_ACTION = 'RequestInventory';
    const REQUEST_INVENTORY_ROOT = 'RequestInventory';

    const VALID_ACTION_NAMES = [
        self::REQUEST_INVENTORY_ACTION,
    ];
     ...
     */
    public function fetchInventoryByEanCode(string $eanCode)
    {
        $data = [
            'Product' => $eanCode
        ];

        return $this->fetchInventory($data);
    }

    /**
     * @return InventoryResponse
     */
    public function fetchTotalInventory()
    {
        $data = [
            'Inventory' => 'True'
        ];
     ...
     * @throws \Exception
     */
    private function fetchInventory(array $data)
    {
        $result = $this->execute(
            $data,
            self::REQUEST_INVENTORY_ACTION,
            self::REQUEST_INVENTORY_ROOT,
        );

        $inventoryResponse = new InventoryResponse($result);
